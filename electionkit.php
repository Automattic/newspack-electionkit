...
function np_sample_ballot() {
	...
	if ( is_wp_error( $bp_districts_request ) ) {
		wp_send_json_error(
			array(
				'message' => "ballotpedia sample ballot elections call didn't work",
			)
		);
	} else {
	...
	if ( ! $bp_district_data ) {
		wp_send_json_error(
			array(
				'message' => "ballotpedia sample ballot elections didn't return any data",
			)
		);
	}
	...
	if ( ! $bp_ballot_data ) {
		wp_send_json_error(
			array(
				'message' => "ballotpedia sample ballot results didn't return any data",
			)
		);
	}
										...
										<div class="candidate-content">
											...
											<div class="social">
												...
												if ( $candidate->person->contact_twitter ) {
													?>
													<a href="https://twitter.com/<?php echo esc_url( $candidate->person->contact_twitter ); ?>" target="_blank" class="icon-twitter">
														<svg viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
